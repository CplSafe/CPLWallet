...
int LightWallet::getDigitsOfPrecision(QString assetSymbol)
{
   ...
   int digits = log10l(OHL_BLOCKCHAIN_PRECISION);
...
void LightWallet::connectToServer(QString host, quint16 port, QString serverKey, QString user, QString password)
{
   ...
   try {
      if( serverKey.isEmpty() )
         ...
         m_wallet.connect(convert(host), convert(user), convert(password), port,
                          ohl::blockchain::public_key_type(convert(serverKey)));
...
QStringList LightWallet::recoverableAccounts(QString brainKey)
{
   ...
   IN_WAIT_THREAD
   int n = 0;
   ohl::blockchain::oaccount_record account;
   do {
      auto key = m_wallet.derive_private_key(convert(brainKey), n++);
      account = m_wallet.get_account_record(std::string(ohl::blockchain::public_key_type(key.get_public_key())));
      if( account )
...
bool LightWallet::recoverWallet(QString accountName, QString password, QString brainKey)
{
   ...
   } catch (ohl::light_wallet::account_already_registered e) {
...
void LightWallet::pollForRegistration(QString accountName)
{
   ...
   while( m_wallet.account(stdAccountName).registration_date == fc::time_point_sec() ) {
      ...
      fc::usleep(fc::seconds(OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC / 2));
   }
}
...
void LightWallet::registerAccount(QString accountName)
{
   ...
   } catch( const ohl::light_wallet::account_already_registered& e ) {
...
void LightWallet::updateAccount(const ohl::blockchain::account_record& account)
{
   ...
   {
      //Tricky threading. We're probably running in the ohlshares thread, but we want Account to belong to the UI thread

...
TransactionSummary* Account::buildSummary(ohl::wallet::transaction_ledger_entry trx)
{
   ...
   for( auto label : trx.delta_labels )
   {
      ...
      for( ohl::blockchain::asset amount : trx.delta_amounts[label.second] )
      {
         ...
         auto asset_yield = std::find_if(yields.begin(), yields.end(),
                                         [amount] (const ohl::blockchain::asset& a) {
   ...
   if( trx.delta_amounts.count("Fee") && trx.delta_amounts["Fee"].size() )
   {
      ...
      if( ohl::blockchain::oasset_record record = m_wallet->get_asset_record(fee.asset_id) )
      {
...
Account::Account(ohl::light_wallet::light_wallet* wallet,
                 const ohl::blockchain::account_record& account,
                 QObject* parent)
   : QObject(parent),
     m_wallet(wallet),
...
Account& Account::operator=(const ohl::blockchain::account_record& account)
{
   m_name = convert(account.name);
   m_ownerKey = convert(std::string(account.owner_key));
   m_activeKey = convert(std::string(account.active_key()));
...
QList<QObject*> Account::transactionHistory(QString asset_symbol)
{
   try {
      ...
      std::sort(raw_transactions.rbegin(), raw_transactions.rend(),
                [](const ohl::wallet::transaction_ledger_entry& a,
                const ohl::wallet::transaction_ledger_entry& b) {
         return a.timestamp < b.timestamp;
      });

      int position = 0;
      for( ohl::wallet::transaction_ledger_entry trx : raw_transactions )
      {

...
public:
   ...
   {
      return ohl::blockchain::chain_database::is_valid_account_name(convert(name));
   }
   Q_INVOKABLE bool isValidKey(QString key)
   {
      try {
         return ohl::blockchain::public_key_type(convert(key)) != ohl::blockchain::public_key_type();
      } catch (...) {
         return false;
      }
   }
   ...
   {
      return OHL_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE;
   }
   QString baseAssetSymbol() const
   {
      return QStringLiteral(OHL_BLOCKCHAIN_SYMBOL);
   }
   QString keyPrefix() const
   {
      return QStringLiteral(OHL_ADDRESS_PREFIX);
   }
...
private:
   ...
   fc::path m_walletPath = fc::path(m_dataDir.toStdWString())
         / "wallet_data.json";
   ohl::light_wallet::light_wallet m_wallet;
   ...
   void updateAccount(const ohl::light_wallet::account_record& account);
};

...
int main(int argc, char*argv[])
{
  for (;;)
  {
    ...
    if (json_as_variant_object.contains("trx"))
    {
      ...
      ohl::client::trx_message trx_msg;
      try
      {
        trx_msg = json_as_variant.as<ohl::client::trx_message>();
      }
      catch (const fc::exception& e)
      {
        elog("Error interpreting json as ohl::client::trx_message: ${what}", ("what", e.to_string()));
      }

      ilog("Parsed ohl::client::trx_message with id ${msg_id} containing transaction ${trx_id}",
        ("msg_id", ohl::net::message(trx_msg).id())("trx_id", trx_msg.trx.id()));
    }
  }

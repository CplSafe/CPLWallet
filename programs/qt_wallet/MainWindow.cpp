    ...
    _clientWrapper(nullptr)
{
  ...
  version = ohl::utilities::git_revision_description;
  // for an official release, git_revision_description is the git tag, which will
  // look like ohl/0.4.28 or dvs/0.4.29
...
void MainWindow::processCustomUrl(QString url)
{
  ...
  else if( components[0].size() > QString(OHL_ADDRESS_PREFIX).size() && components[0].startsWith(OHL_ADDRESS_PREFIX) )
  {
    //This is a key.
  }
  else if( components[0].size() >= OHL_BLOCKCHAIN_MIN_SYMBOL_SIZE
           && components[0].size() <= OHL_BLOCKCHAIN_MAX_SYMBOL_SIZE
...
void MainWindow::goToBlock(QString blockId)
{
  ...
  try
  {
    auto block = _clientWrapper->get_client()->get_chain()->get_block_digest(ohl::blockchain::block_id_type(blockId.toStdString()));
...
void MainWindow::goToTransaction(QString transactionId)
{
  ...
  try {
    ...
    error = true;
    if (e.code() == ohl::wallet::invalid_transaction_id().code())
      errorDialog.setText(tr("The provided ID is not a valid transaction ID."));
    else if (e.code() == ohl::wallet::transaction_not_found().code())
...
bool MainWindow::walletIsUnlocked(bool promptToUnlock)
{
  ...
  while( promptToUnlock )
  {
    ...
    if( promptToUnlock )
    {
      try
      {
        _clientWrapper->get_client()->get_wallet()->unlock( password.toStdString(), OHL_WALLET_DEFAULT_UNLOCK_TIME_SEC );
...
void MainWindow::doLogin(QStringList components)
{
  try
  {
    ...
    ohl::blockchain::public_key_type serverOneTimeKey;

    try {
      serverOneTimeKey = fc::variant(components[0].toStdString()).as<ohl::blockchain::public_key_type>();
    } catch (const fc::exception& e) {
    ...
    query.addQueryItem("client_key",  fc::variant(ohl::blockchain::public_key_type(myOneTimeKey.get_public_key())).as_string().c_str());
  ...
  if(ok) {
    ...
    try {
      ...
      QMessageBox::critical(this,
                            ...
                            tr("Failed to restore wallet backup. Your original wallet has been restored. Error: %1If you are sure that your password and backup file are correct, please post a support request here: https://ohlsharestalk.org/index.php/board,45.0.html").arg(e.to_string().c_str()));
    }
  } else return;
...
bool MainWindow::verifyUpdateSignature (QByteArray updatePackage)
{
  ...
  if (_webUpdateDescription.timestamp < fc::time_point_sec(ohl::utilities::git_revision_unix_timestamp)) {
  ...
  for (auto signature : _webUpdateDescription.signatures)
    authorized_signers.erase(ohl::blockchain::address(fc::ecc::public_key(signature, hash, false)));

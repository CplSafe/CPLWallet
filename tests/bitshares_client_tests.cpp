...
#include <ohl/rpc/rpc_client.hpp>
#include <ohl/blockchain/asset.hpp>
...
#include <ohl/net/message.hpp>
#include <ohl/net/config.hpp>

#define OHL_BLOCKCHAIN_INITIAL_SHARES (OHL_BLOCKCHAIN_MAX_SHARES/5)

using namespace ohl::utilities;
using namespace ohl::blockchain;
using namespace ohl::wallet;

////////////////////////////////////////////////////////////////////////////////
// Parse the command line for configuration options here ///////////////////////
struct ohl_xt_client_test_config
{
  static fc::path ohl_client_exe;
  static fc::path config_directory;
  static uint16_t base_rpc_port;
  static uint16_t base_p2p_port;
  static uint16_t base_http_port;

  ohl_xt_client_test_config()
  {
    // parse command-line options
    boost::program_options::options_description option_config("Allowed options");
    option_config.add_options()("ohl-client-exe", boost::program_options::value<std::string>(), "full path to the executable to test")
    ...
    if (option_variables.count("ohl-client-exe"))
      ohl_client_exe = option_variables["ohl-client-exe"].as<std::string>().c_str();

    std::cout << "Testing " << ohl_client_exe.string() << "\n";
...
fc::path ohl_xt_client_test_config::ohl_client_exe = "e:/invictus/vs12_bt/programs/client/Debug/ohlshares_client.exe";
fc::path ohl_xt_client_test_config::config_directory = fc::temp_directory_path() / "ohlshares_client_tests";
uint16_t ohl_xt_client_test_config::base_rpc_port  = 20100;
uint16_t ohl_xt_client_test_config::base_p2p_port  = 21100;
uint16_t ohl_xt_client_test_config::base_http_port = 22100;
...
BOOST_GLOBAL_FIXTURE(ohl_xt_client_test_config);
...
struct ohl_client_process : managed_process
{
  ...
  uint16_t p2p_port;
  uint16_t http_port;
  ohl::rpc::rpc_client_ptr rpc_client;
  ohl::net::node_id_t node_id;
  fc::path config_dir;
  ohl::wallet::wallet_ptr wallet; // used during initial creation, closed after that
  ohl::blockchain::chain_database_ptr blockchain;

  ohl::blockchain::share_type expected_balance; // for use in individual tests to track the expected balance after a transfer

  ohl_client_process() : initial_balance(0) {}
  void set_process_number(uint32_t process_num)
  {
    process_number = process_num;
    std::ostringstream numbered_config_dir_name;
    numbered_config_dir_name << "OhlSharesX_" << std::setw(3) << std::setfill('0') << process_number;
    config_dir = ohl_xt_client_test_config::config_directory / numbered_config_dir_name.str();
    fc::remove_all(config_dir);
    fc::create_directories(config_dir);
  }
  void launch(uint32_t process_number,
              fc::optional<ohl::blockchain::genesis_block_config> genesis_block);
};
typedef std::shared_ptr<ohl_client_process> ohl_client_process_ptr;

void ohl_client_process::launch(uint32_t process_number,
                                fc::optional<ohl::blockchain::genesis_block_config> genesis_block)
{
  ...
  // we could (and probably should) make ohl_xt_client create the wallet,
  ...
  {
    ohl::wallet::wallet_ptr wallet = std::make_shared<ohl::wallet::wallet>();
...
#endif
    launch_script << ohl_xt_client_test_config::ohl_client_exe.string() << " " << boost::algorithm::join(options, " ") << "\n";
  }

  process->exec(ohl_xt_client_test_config::ohl_client_exe, options, config_dir);

  log_stdout_stderr_to_file(config_dir / "stdouterr.txt");
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

struct ohl_client_launcher_fixture
{
  std::vector<ohl_client_process> client_processes;
  std::vector<fc::ecc::private_key> delegate_keys;

  ohl::blockchain::genesis_block_config genesis_block;
  ...
  ohl_client_launcher_fixture() :
    _peer_connection_retry_timeout(15 /* sec */),
    _desired_number_of_connections(OHL_MIN_DELEGATE_CONNECTION_COUNT + 2),
    _maximum_number_of_connections(OHL_MIN_DELEGATE_CONNECTION_COUNT * 2)
  {}
  ...
  void get_node_ids();
  void create_propagation_graph(const std::vector<ohl::net::message_propagation_data>& propagation_data, int initial_node, const fc::path& output_file);
};

void ohl_client_launcher_fixture::create_delegates_and_genesis_block()
{
  // generate a genesis block giving 100ohl to each account
  ...
  for (unsigned i = 0; i < client_processes.size(); ++i)
  {
    ...
    genesis_block.balances.push_back(std::make_pair(ohl::blockchain::pts_address(client_processes[i].private_key.get_public_key()),
                                                    (double)client_processes[i].initial_balance));
    initial_shares_requested += client_processes[i].initial_balance;
  }

  double scale_factor = OHL_BLOCKCHAIN_INITIAL_SHARES / (double)initial_shares_requested;
  for (unsigned i = 0; i < client_processes.size(); ++i)
    client_processes[i].initial_balance *= (int64_t)scale_factor;

  for (unsigned i = 0; i < OHL_BLOCKCHAIN_NUM_DELEGATES; ++i)
  {
    ohl::blockchain::name_config delegate_config;
...
void ohl_client_launcher_fixture::create_unsynchronized_wallets()
{
  const uint32_t initial_block_count = 400; // generate this many blocks
  ohl::blockchain::start_simulated_time(fc::time_point_sec(0)); // restore real time
  fc::time_point_sec genesis_block_time(((ohl::blockchain::now().sec_since_epoch() / OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC) - (initial_block_count + 1)) * OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC);

  ohl::blockchain::start_simulated_time(genesis_block_time);
  ...
  fc::path genesis_json = ohl_xt_client_test_config::config_directory / "genesis.json";
  fc::json::save_to_file(genesis_block, genesis_json, true);

  ohl::blockchain::chain_database_ptr master_blockchain = std::make_shared<ohl::blockchain::chain_database>();
  fc::path master_data_dir = ohl_xt_client_test_config::config_directory / "master_blockchain";
  fc::remove_all(master_data_dir);
  master_blockchain->open(master_data_dir / "chain", genesis_json);
  ohl::wallet::wallet_ptr master_wallet = std::make_shared<ohl::wallet::wallet>(master_blockchain);
  ...
  {
    ohl::blockchain::advance_time(OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
    ohl::blockchain::full_block next_block = master_blockchain->generate_block(ohl::blockchain::now());
  ...
  for (unsigned client_index = 0; client_index < client_processes.size(); ++client_index)
  {
    ...
    client_processes[client_index].blockchain = std::make_shared<ohl::blockchain::chain_database>();
    client_processes[client_index].blockchain->open(client_processes[client_index].config_dir / "chain", genesis_json);
    client_processes[client_index].wallet = std::make_shared<ohl::wallet::wallet>(client_processes[client_index].blockchain);
  ...
  ohl::blockchain::start_simulated_time(fc::time_point_sec(0)); // restore real time
  BOOST_REQUIRE(head_block_timestamp <= ohl::blockchain::now());
  BOOST_TEST_MESSAGE("Last generated block's timestamp: " << (std::string)fc::time_point(head_block_timestamp) << ", wall clock time is " << (std::string)fc::time_point(ohl::blockchain::now()));
}

void ohl_client_launcher_fixture::create_forked_wallets()
{
  const uint32_t initial_block_count = 400; // generate this many blocks
  fc::time_point_sec genesis_block_time(((ohl::blockchain::now().sec_since_epoch() / OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC) - (initial_block_count + 1)) * OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
  ohl::blockchain::advance_time(genesis_block_time.sec_since_epoch() - ohl::blockchain::now().sec_since_epoch());
  ...
  for (unsigned i = 0; i < client_processes.size(); ++i)
  {
    ...
    client_processes[i].blockchain = std::make_shared<ohl::blockchain::chain_database>();
    client_processes[i].blockchain->open(client_processes[i].config_dir / "chain", genesis_json);
    client_processes[i].wallet = std::make_shared<ohl::wallet::wallet>(client_processes[i].blockchain);
  ...
  ohl_client_process& first_client = client_processes[0];
  ...
  for (current_block_count = 0; current_block_count < initial_block_count; ++current_block_count)
  {
    ...
    ohl::blockchain::advance_time(OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
    std::vector<ohl::wallet::wallet_account_record> enabled_delegates = first_client.wallet->get_my_delegates( enabled_delegate_status );
    ...
    ohl::blockchain::advance_time(my_next_block_time.sec_since_epoch() - ohl::blockchain::now().sec_since_epoch());
    ohl::blockchain::full_block next_block = first_client.blockchain->generate_block(my_next_block_time);
...
void ohl_client_launcher_fixture::launch_client(uint32_t client_index)
{
  client_processes[client_index].rpc_port = ohl_xt_client_test_config::base_rpc_port + client_index;
  client_processes[client_index].p2p_port = ohl_xt_client_test_config::base_p2p_port + client_index;
  client_processes[client_index].http_port = ohl_xt_client_test_config::base_http_port + client_index;
  fc::optional<ohl::blockchain::genesis_block_config> optional_genesis_block;
...
void ohl_client_launcher_fixture::launch_clients()
{
  BOOST_TEST_MESSAGE("Launching " << client_processes.size() << " ohl_xt_client processes");
  for (unsigned i = 0; i < client_processes.size(); ++i)
    launch_client(i);
}

void ohl_client_launcher_fixture::establish_rpc_connection(uint32_t client_index)
{
  client_processes[client_index].rpc_client = std::make_shared<ohl::rpc::rpc_client>();
...
void ohl_client_launcher_fixture::establish_rpc_connections()
{
...
void ohl_client_launcher_fixture::trigger_network_connections()
{
  ...
  for (unsigned i = 0; i < client_processes.size(); ++i)
  {
    ...
    client_processes[i].rpc_client->network_add_node(fc::ip::endpoint(fc::ip::address("127.0.0.1"), ohl_xt_client_test_config::base_p2p_port), "add");
    fc::usleep(fc::milliseconds(500));
  }
}

void ohl_client_launcher_fixture::import_initial_balances()
{
...
void ohl_client_launcher_fixture::register_delegates()
{
  for (unsigned i = 0; i < delegate_keys.size(); ++i)
  {
    std::ostringstream delegate_name;
...
int ohl_client_launcher_fixture::verify_network_connectivity(const fc::path& output_path)
{
  ...
  for (unsigned i = 0; i < client_processes.size(); ++i)
  {
    ...
    for (const fc::variant& peer_info : peers_info)
    {
      ...
      if (peer_info_object["inbound"].as_bool())
      {
        ...
        boost::add_edge(peer_p2p_port - ohl_xt_client_test_config::base_p2p_port, i, _directed_graph);
      }
    }
...
void ohl_client_launcher_fixture::get_node_ids()
{
  for (unsigned i = 0; i < client_processes.size(); ++i)
    BOOST_CHECK_NO_THROW(client_processes[i].node_id = client_processes[i].rpc_client->network_get_info()["node_id"].as<ohl::net::node_id_t>());
}

void ohl_client_launcher_fixture::create_propagation_graph(const std::vector<ohl::net::message_propagation_data>& propagation_data, int initial_node, const fc::path& output_file)
{
  struct graph_property_writer
  {
    void operator()(std::ostream& out) const
  ...
  std::map<ohl::net::node_id_t, int> node_id_to_node_map;
  for (unsigned i = 0; i < client_processes.size(); ++i)
    node_id_to_node_map[client_processes[i].node_id] = i;

  class vertex_label_writer {
  public:
    vertex_label_writer(const std::vector<ohl::net::message_propagation_data>& propagation_data, int initial_node, fc::microseconds max_duration) :
  ...
  private:
    const std::vector<ohl::net::message_propagation_data>& _propagation_data;
    int _initial_node;
  ...
  public:
    edge_label_writer(const std::vector<ohl::net::message_propagation_data>& propagation_data,
                      int initial_node,
  ...
  private:
    ...
    fc::microseconds _max_duration;
    DirectedGraph& _directed_graph;
    std::map<ohl::net::node_id_t, int> _node_id_to_node_map;
  };
...
BOOST_FIXTURE_TEST_SUITE(ohl_xt_client_test_suite, ohl_client_launcher_fixture)
...
BOOST_AUTO_TEST_CASE(standalone_wallet_test)
{
  ...
  for (unsigned i = 0; i < client_processes.size(); ++i)
  {
    std::vector<ohl::wallet::wallet_account_record> initial_receive_accounts = client_processes[i].rpc_client->wallet_list_my_accounts();
    BOOST_CHECK(initial_receive_accounts.empty());
    std::string account_name("address_test_account");
    //ohl::blockchain::public_key_type new_address = client_processes[i].rpc_client->wallet_account_create(account_name);
    std::vector<ohl::wallet::wallet_account_record> final_receive_accounts = client_processes[i].rpc_client->wallet_list_my_accounts();
...
BOOST_AUTO_TEST_CASE(transfer_test)
{
  ...
  verify_network_connectivity(ohl_xt_client_test_config::config_directory / "transfer_test");
  ...
  BOOST_TEST_MESSAGE("Sending 1 million OHL to the next client in the list");
  for (unsigned i = 0; i < client_processes.size(); ++i)
  {
    uint32_t next_client_index = (i + 1) % client_processes.size();
    ohl::blockchain::public_key_type destination_address = client_processes[next_client_index].rpc_client->wallet_account_create("circletest");
    ohl::blockchain::share_type destination_initial_balance = client_processes[next_client_index].rpc_client->wallet_account_balance()["circletest"].first["XTS"];
    //ohl::blockchain::asset source_initial_balance = client_processes[i].rpc_client->wallet_get_balance();
    ohl::blockchain::share_type amount_to_transfer = 10;
    client_processes[i].rpc_client->wallet_add_contact_account("nextclient", destination_address);
    ohl::blockchain::signed_transaction transaction = client_processes[i].rpc_client->wallet_transfer(amount_to_transfer, "XTS", INITIAL_BALANCE_ACCOUNT, "nextclient");
    fc::time_point transfer_time = fc::time_point::now();

    fc::usleep(fc::seconds(2));

    std::vector<ohl::net::message_propagation_data> propagation_data;
    ...
    verify_network_connectivity(ohl_xt_client_test_config::config_directory / "transfer_test" / graph_filename.str());
    create_propagation_graph(propagation_data, i, ohl_xt_client_test_config::config_directory / "transfer_test" / propagation_graph_filename.str());

    for (;;)
    {
      fc::usleep(fc::milliseconds(500));
      int64_t precision = client_processes[next_client_index].rpc_client->blockchain_get_asset("XTS")->precision;
      ohl::blockchain::share_type new_account_balance = client_processes[next_client_index].rpc_client->wallet_account_balance()["circletest"].first["XTS"];
      if (new_account_balance == destination_initial_balance + amount_to_transfer * precision)
      {
        BOOST_TEST_MESSAGE("Client " << next_client_index << " received 1MOHL from client " << i);
        break;
      }
      if (fc::time_point::now() > transfer_time + fc::seconds(OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC + 5))
...
BOOST_AUTO_TEST_CASE(thousand_transactions_per_block)
{
  ...
  std::vector<ohl::blockchain::public_key_type> recieve_addresses;
...
BOOST_AUTO_TEST_CASE(untracked_transactions)
{
  ...
  int number_of_partitions = verify_network_connectivity(ohl_xt_client_test_config::config_directory / "untracked_transactions");
...
BOOST_AUTO_TEST_CASE(simple_sync_test)
{
  ...
  int number_of_partitions = verify_network_connectivity(ohl_xt_client_test_config::config_directory / "simple_sync_test" / "network_map.dot");
  BOOST_REQUIRE_EQUAL(number_of_partitions, 1);
...
BOOST_AUTO_TEST_CASE(net_split_test)
{
  ...
  int partition_count = verify_network_connectivity(ohl_xt_client_test_config::config_directory / "net_split_test" / "pre_split_map.dot");
  BOOST_CHECK_EQUAL(partition_count, 1);

  std::vector<ohl::net::node_id_t> even_node_ids;
  std::vector<ohl::net::node_id_t> odd_node_ids;
  for (unsigned i = 0; i < client_processes.size(); ++i)
    if (i % 2 == 0)
  ...
  partition_count = verify_network_connectivity(ohl_xt_client_test_config::config_directory / "net_split_test" / "post_split_map.dot");
...
BOOST_AUTO_TEST_CASE(simple_fork_resolution_test)
{
  ...
  int partition_count = verify_network_connectivity(ohl_xt_client_test_config::config_directory / "simple_fork_resolution_test" / "pre_split_map.dot");
  BOOST_CHECK_EQUAL(partition_count, 1);
  ...
  int number_of_partitions = verify_network_connectivity(ohl_xt_client_test_config::config_directory / "simple_fork_resolution_test" / "post_split_map.dot");
  ...
  fc::usleep(fc::seconds(OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 11));
  ...
  for (int loop = 0; loop < 5; ++loop)
  {
    ...
    std::vector<ohl::blockchain::block_id_type> block_hashes_after_fork;
    ...
    if (even_are_same && odd_are_same)
    {
      ...
      else
      {
        ...
        fc::usleep(fc::seconds(OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC));
      }
    }
  }
  BOOST_REQUIRE(clients_in_sync);
  ...
  for (unsigned i = 0; i < client_processes.size(); ++i)
    client_processes[i].rpc_client->network_set_allowed_peers(std::vector<ohl::net::node_id_t>());
  for (unsigned i = 0; i < client_processes.size(); ++i)
    client_processes[i].rpc_client->network_add_node(fc::ip::endpoint(fc::ip::address("127.0.0.1"), ohl_xt_client_test_config::base_p2p_port + ((i + 1) % 2)), "add");
  BOOST_TEST_MESSAGE("Reconnection triggered, now we wait");
  fc::usleep(fc::seconds(_peer_connection_retry_timeout * 2));
  number_of_partitions = verify_network_connectivity(ohl_xt_client_test_config::config_directory / "simple_fork_resolution_test" / "joined_network_map.dot");
  BOOST_REQUIRE_EQUAL(number_of_partitions, 1);
  ...
  for (int loop = 0; loop < 5; ++loop)
  {
    std::vector<ohl::blockchain::block_id_type> block_hashes_after_net_join;
...
BOOST_AUTO_TEST_CASE(fifty_node_test)
{
  ...
  verify_network_connectivity(ohl_xt_client_test_config::config_directory / "fifty_node_test");
  ...
  //ohl::blockchain::public_key_type receive_address = client_processes[0].rpc_client->wallet_account_create("test");
  ohl::blockchain::transaction_id_type transaction_id;// = client_processes[0].rpc_client->wallet_transfer(receive_address, 50);
  ...
  create_propagation_graph(propagation_data, 0, ohl_xt_client_test_config::config_directory / "fifty_node_test" / "transaction_propagation.dot");
...
BOOST_AUTO_TEST_CASE(test_with_mild_churn)
{
  ...
  int partition_count = verify_network_connectivity(ohl_xt_client_test_config::config_directory / "test_with_mild_churn" / "initial_map.dot");
  ...
  for (uint32_t i = 0; i < test_duration_in_blocks; ++i)
  {
    ...
    for (unsigned i = 0; i < transactions_to_send_each_block; ++i)
    {
      ...
      //ohl::blockchain::public_key_type receive_address = client_processes[dest].rpc_client->wallet_account_create(receive_note.str());
    ...
    int partition_count = verify_network_connectivity(ohl_xt_client_test_config::config_directory / "test_with_mild_churn" / dot_filename.str());
    BOOST_CHECK_EQUAL(partition_count, 1);
  }
  for (unsigned i = 0; i < client_processes.size(); ++i)
    client_processes[i].rpc_client->stop();
...
ohl::net::message garbage_message(int size)
{
...
BOOST_AUTO_TEST_CASE( oversize_message_test )
{
    ...
    ohl::net::message oversize = garbage_message( MAX_MESSAGE_SIZE * 500 ) ;
...
{
    auto k1 = ohl::blockchain::public_key_type( fc::ecc::private_key::generate().get_public_key() );
    auto k2 = ohl::blockchain::public_key_type( fc::ecc::private_key::generate().get_public_key() );
    auto k3 = ohl::blockchain::public_key_type( fc::ecc::private_key::generate().get_public_key() );
    BOOST_VERIFY(k1 == ohl::blockchain::public_key_type( std::string( k1 ) ));
    BOOST_VERIFY(k2 == ohl::blockchain::public_key_type( std::string( k2 ) ));
    BOOST_VERIFY(k3 == ohl::blockchain::public_key_type( std::string( k3 ) ));
}

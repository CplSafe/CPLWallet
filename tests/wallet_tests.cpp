...
#include <ohl/client/api_logger.hpp>
...
using namespace ohl::client;
using namespace ohl::cli;
...
BOOST_AUTO_TEST_CASE( wif_format_test )
{
  try {
   ...
   FC_ASSERT( ohl::utilities::key_to_wif(priv_key) == "5HueCGU8rMjxEXxiPuD5BDku4MkFqeZyd4dZ1jvhTVqvbTLvyTJ" );
   FC_ASSERT( ohl::utilities::wif_to_key( "5HueCGU8rMjxEXxiPuD5BDku4MkFqeZyd4dZ1jvhTVqvbTLvyTJ" ).valid() );
...
void produce_block( T my_client )
{
      ...
      ohl::blockchain::advance_time( (int32_t)((*next_block_time - ohl::blockchain::now()).count()/1000000) );
      auto b = my_client->get_chain()->generate_block(*next_block_time);
      my_client->get_wallet()->sign_block( b );
      my_client->get_node()->broadcast( ohl::client::block_message( b ) );
      FC_ASSERT( head_num+1 == my_client->get_chain()->get_head_block_num() );
      ohl::blockchain::advance_time( 1 );
}
...
void create_genesis_block(fc::path genesis_json_file)
{
   ...
   config.timestamp = ohl::blockchain::now();

   // set our fake random number generator to generate deterministic keys
   ohl::utilities::set_random_seed_for_testing( fc::sha512::hash( string( "genesis" ) ) );
   ...
   for( uint32_t i = 0; i < OHL_BLOCKCHAIN_NUM_DELEGATES; ++i )
   {
      ...
      balance.balance = double( OHL_BLOCKCHAIN_INITIAL_SHARES ) / OHL_BLOCKCHAIN_NUM_DELEGATES;
      config.initial_balances.push_back( balance );

      //output public/private key pair for each delegate to a file
      string wif_key = ohl::utilities::key_to_wif( delegate_private_key );
...
{
  ohl::blockchain::start_simulated_time(fc::time_point_sec::min());
  // set our fake random number generator to generate deterministic keys
  ohl::utilities::set_random_seed_for_testing( fc::sha512::hash( string( "regression" ) ) );
  ...
  // caller of this routine should have made sure we are already in ohlshares/test dir.
  // so we pop dirs to create regression_tests_results as sibling to ohlshares source directory
  ...
  try
  {
    ...
    ohl::client::stream_api_logger glogger(test_glog_file);
    glogger.connect();
#endif

    //create one client per line and run each client's input commands
    auto sim_network = std::make_shared<ohl::net::simulated_network>("wallet_tests");
    vector<test_file> tests;
    string line;
    std::vector<ohl::client::client_ptr> clients;
    while (std::getline(test_config_file, line))
    {
      ...
      //if no data-dir specified, put in ../ohlshares/regression_tests/${test dir}/${client_name}
      ...
      auto option_variables = ohl::client::parse_option_variables(argc, argv);
    #endif
      ...
      {
        ohl::client::client_ptr client = std::make_shared<ohl::client::client>("wallet_tests", sim_network);
  ...
  * 
  * https://github.com/OhlShares/ohlshares/issues/1487
  */
...
{
  const uint32_t granularity = OHL_BLOCKCHAIN_BLOCKS_PER_DAY;

  fc::temp_directory client_dir;
  fc::remove_all(client_dir.path());
  //auto sim_network = std::make_shared<ohl::net::simulated_network>();
  ohl::net::simulated_network_ptr sim_network = std::make_shared<ohl::net::simulated_network>("wallet_tests");
  ohl::client::client_ptr client = std::make_shared<ohl::client::client>("wallet_tests", sim_network);
  //client->open( client_dir.path() );
  std::string client_dir_string = client_dir.path().string();
  char *fake_argv[] = {"ohlshares_client", "--data-dir", (char*)client_dir_string.c_str(), "--accept-incoming-connections=false", "--disable-default-peers", "--upnp=false"};
  ...
  ohl::blockchain::chain_database_ptr source_blockchain = std::make_shared<ohl::blockchain::chain_database>();
  fc::path test_net_chain_dir("C:\\Users\\Administrator\\AppData\\Roaming\\OhlShares X");
  ...
  std::vector<ohl::client::block_message> all_messages;
  ...
  client->sync_status(ohl::client::block_message::type, total_blocks_to_replay);
  ...
  client->sync_status(ohl::client::block_message::type, 0);
...
void replay_chain_database_in_stages()
{
  ...
  for (int n = 0; n < number_of_stages; ++n)
  {
    ohl::client::client_ptr client = std::make_shared<ohl::client::client>("wallet_tests", sim_network);
    std::string client_dir_string = client_dir.path().string();
    char *fake_argv[] = {"ohlshares_client", "--data-dir", (char*)client_dir_string.c_str(), "--accept-incoming-connections=false", "--disable-default-peers", "--upnp=false", "--daemon", "--rpcuser=none", "--rpcpassword=none"};
    client->configure_from_command_line(sizeof(fake_argv) / sizeof(fake_argv[0]), fake_argv);
    fc::future<void> client_done = client->start();
    ...
    client->sync_status(ohl::client::block_message::type, end_block_num - start_block_num);

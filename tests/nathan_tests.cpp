...
{ try {
    ...
    asset owed = ohl::blockchain::detail::market_engine::get_interest_owed(principle, apr, loan_age);
    wlog( "${x}", ("x",owed) );
    ...
    owed = ohl::blockchain::detail::market_engine::get_interest_owed(principle, apr, loan_age);
    wlog( "${x}", ("x",owed) );
    BOOST_CHECK_EQUAL( owed.amount, chain->to_ugly_asset("50", "USD").amount );

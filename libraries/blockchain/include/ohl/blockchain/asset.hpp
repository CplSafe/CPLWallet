...
namespace ohl { namespace blockchain {
  ...
  struct price
  {
      ...
      fc::uint128_t ratio; // This is a DECIMAL FRACTION with denominator equal to OHL_PRICE_PRECISION
   ...
   *  denominator.  Therefore  ohl/usd and  usd/ohl will
   *  always result in a price measured in usd/ohl because
   ...
   *
   *  ie:  p = 3 usd/ohl & a = 4 ohl then result = 12 usd
   *  ie:  p = 3 usd/ohl & a = 4 usd then result = 1.333 ohl
   */
...
namespace fc
{
 //  void to_variant( const ohl::blockchain::asset& var,  variant& vo );
 //  void from_variant( const variant& var,  ohl::blockchain::asset& vo );
   void to_variant( const ohl::blockchain::price& var,  variant& vo );
   void from_variant( const variant& var,  ohl::blockchain::price& vo );
}

#include <fc/reflect/reflect.hpp>
FC_REFLECT( ohl::blockchain::asset, (amount)(asset_id) );
FC_REFLECT( ohl::blockchain::price, (ratio)(quote_asset_id)(base_asset_id) );

...
namespace ohl { namespace blockchain {
   ...
   class address
   {
      public:
       ...
       static bool is_valid( const std::string& base58str, const std::string& prefix = OHL_ADDRESS_PREFIX );
...
namespace fc
{
   void to_variant( const ohl::blockchain::address& var,  fc::variant& vo );
   void from_variant( const fc::variant& var,  ohl::blockchain::address& vo );
}

namespace std
{
   template<>
   struct hash<ohl::blockchain::address>
   {
       public:
         size_t operator()(const ohl::blockchain::address &a) const
         {
...
FC_REFLECT( ohl::blockchain::address, (addr) )

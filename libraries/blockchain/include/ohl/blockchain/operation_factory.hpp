       ...
       public:
          ...
          class operation_converter_base
          {
             public:
                  ...
                  virtual void to_variant( const ohl::blockchain::operation& in, fc::variant& out ) = 0;
                  virtual void from_variant( const fc::variant& in, ohl::blockchain::operation& out ) = 0;
          ...
          {
             public:
                  virtual void to_variant( const ohl::blockchain::operation& in, fc::variant& output )
                  { try {
                  ...
                  virtual void from_variant( const fc::variant& in, ohl::blockchain::operation& output )
                  { try {
          ...
          void evaluate( transaction_evaluation_state& eval_state, const operation& op )
          {
             ...
             if( itr == _converters.end() )
                FC_THROW_EXCEPTION( ohl::blockchain::unsupported_chain_operation, "", ("op",op) );
             itr->second->evaluate( eval_state, op );
          }

          /// defined in operations.cpp
          void to_variant( const ohl::blockchain::operation& in, fc::variant& output );
          /// defined in operations.cpp
          void from_variant( const fc::variant& in, ohl::blockchain::operation& output );

...
namespace ohl { namespace blockchain {
   ...
   struct memo_data
   {
       ...
       */
      fc::array<char,OHL_BLOCKCHAIN_MAX_MEMO_SIZE>     message;
   ...
   struct extended_memo_data
   {
      ...
      fc::array<char,OHL_BLOCKCHAIN_MAX_MEMO_SIZE>      message;
      fc::enum_type<uint8_t,memo_flags_enum>            memo_flags = from_memo;
      fc::array<char,OHL_BLOCKCHAIN_EXTENDED_MEMO_SIZE> extra_message;
   };
...
namespace fc {
   void to_variant( const ohl::blockchain::withdraw_condition& var,  variant& vo );
   void from_variant( const variant& var,  ohl::blockchain::withdraw_condition& vo );
   void to_variant( const ohl::blockchain::memo_data& var,  variant& vo );
   void from_variant( const variant& var,  ohl::blockchain::memo_data& vo );
}

FC_REFLECT_ENUM( ohl::blockchain::withdraw_condition_types,
        (withdraw_null_type)
...
FC_REFLECT( ohl::blockchain::withdraw_condition,
        (asset_id)
        (slate_id)
        (type)
        (data)
        )
FC_REFLECT( ohl::blockchain::titan_memo,
        (one_time_key)
        (encrypted_memo_data)
        )
FC_REFLECT_ENUM( ohl::blockchain::memo_flags_enum,
        (from_memo)
        (to_memo)
        )
FC_REFLECT( ohl::blockchain::memo_data,
        (from)
        (from_signature)
        (message)
        (memo_flags)
...
FC_REFLECT( ohl::blockchain::extended_memo_data,
        (from)
        (from_signature)
        (message)
        (memo_flags)
...
FC_REFLECT_DERIVED( ohl::blockchain::memo_status,
        (ohl::blockchain::memo_data),
        (has_valid_signature)
        (owner_private_key)
        )
FC_REFLECT( ohl::blockchain::withdraw_with_signature,
        (owner)
        (memo)
        )
FC_REFLECT( ohl::blockchain::withdraw_vesting,
        (owner)
        (start_time)
        (duration)
        (original_balance)
        )
FC_REFLECT( ohl::blockchain::withdraw_with_multisig,
        (required)
        (owners)
        (memo)
        )
FC_REFLECT( ohl::blockchain::withdraw_with_escrow,
        (sender)
        (receiver)
        (escrow)
        (agreement_digest)

...
#include <ohl/blockchain/pending_chain_state.hpp>
...
namespace ohl { namespace blockchain {
   ...
   asset balance_record::calculate_yield( fc::time_point_sec now, share_type amount, share_type yield_pool, share_type share_supply )const
   {
      ...
      if( elapsed_time <= fc::seconds( OHL_BLOCKCHAIN_MIN_YIELD_PERIOD_SEC ) )
      ...
      // OHL_BLOCKCHAIN_MAX_SHARES * 1000000 * OHL_BLOCKCHAIN_MAX_SHARES
       ...
       *  interest.
       */
      if( elapsed_time < fc::seconds( OHL_BLOCKCHAIN_MAX_YIELD_PERIOD_SEC ) )
      {
         ...
         // OHL_BLOCKCHAIN_MAX_SHARES * 1000000 * OHL_BLOCKCHAIN_MAX_SHARES * 8
         // thus it cannot overflow
         //
         ...
         yield /= OHL_BLOCKCHAIN_MAX_YIELD_PERIOD_SEC;

         delta_yield *= elapsed_time.to_seconds();
         delta_yield /= OHL_BLOCKCHAIN_MAX_YIELD_PERIOD_SEC;
   ...
   { try {
       if( eval_state.pending_state()->get_head_block_num() < OHL_V0_9_0_FORK_BLOCK_NUM )
   ...
   { try {
      if( eval_state.pending_state()->get_head_block_num() < OHL_V0_4_21_FORK_BLOCK_NUM )
   ...
   { try {
      ...
      if( amount.asset_id == 0 )
      {
          ...
          const share_type required_fee = message_kb * OHL_BLOCKCHAIN_MIN_BURN_FEE;
   ...
   { try {
      ...
      FC_ASSERT( current_balance_record->condition.asset_id == 0, "Only OHL balances can have restricted owners." );
      ...
      auto fee = OHL_BLOCKCHAIN_PRECISION / 2;
      ...
      else // NOT this->new_restricted_owner.valid() || (this->new_restricted_owner == new_restricted_owner)
      {
          ...
          FC_ASSERT( eval_state.pending_state()->now().sec_since_epoch() - last_update_secs
                     >= OHL_BLOCKCHAIN_VOTE_UPDATE_PERIOD_SEC,

...
namespace ohl { namespace blockchain {
  ...
  fc::ecc::public_key extended_private_key::get_public_key()const
  {
   ...
   extended_address::operator std::string()const
   {
      ...
      return OHL_ADDRESS_PREFIX + fc::to_base58( data.data(), data.size() );
   }
...
namespace fc
{
   void to_variant( const ohl::blockchain::extended_address& ext_addr,  fc::variant& vo )
   {
   ...
   {
      vo = ohl::blockchain::extended_address( var.as_string() );
   }
}

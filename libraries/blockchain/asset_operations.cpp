...
{ try {
    ...
    if( this->maximum_share_supply <= 0 || this->maximum_share_supply > OHL_BLOCKCHAIN_MAX_SHARES )
...
{ try {
    ...
    if( this->max_supply.valid() )
    {
        ...
        if( *this->max_supply < current_asset_record->current_supply || *this->max_supply > OHL_BLOCKCHAIN_MAX_SHARES )
    ...
    if( this->market_fee_rate.valid() )
    {
        ...
        if( *this->market_fee_rate > OHL_BLOCKCHAIN_MAX_UIA_MARKET_FEE_RATE )

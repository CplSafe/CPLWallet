...
#define OHL_BLOCKCHAIN_DELEGATE_REGISTRATION_FEE    (OHL_BLOCKCHAIN_BLOCKS_PER_DAY / 12)
#define OHL_BLOCKCHAIN_ASSET_REGISTRATION_FEE       (OHL_BLOCKCHAIN_BLOCKS_PER_DAY * 14)
...
share_type chain_interface::get_delegate_registration_fee_v1( uint8_t pay_rate )const
{
   ...
   const share_type unscaled_fee = (get_delegate_pay_rate_v1() * OHL_BLOCKCHAIN_DELEGATE_REGISTRATION_FEE)/OHL_BLOCKCHAIN_NUM_DELEGATES;
   return (unscaled_fee * pay_rate) / 100;
}

share_type chain_interface::get_asset_registration_fee_v1()const
{
   return (get_delegate_pay_rate_v1() * OHL_BLOCKCHAIN_ASSET_REGISTRATION_FEE);
}
...
share_type chain_interface::get_delegate_pay_rate_v1()const
{
    ...
    return base_record->collected_fees / (OHL_BLOCKCHAIN_BLOCKS_PER_DAY * 14);
}

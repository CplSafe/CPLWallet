...
#include <ohl/blockchain/block_record.hpp>
#include <ohl/blockchain/asset_record.hpp>
#include <ohl/blockchain/chain_database.hpp>
#include <ohl/rpc/rpc_client.hpp>
#include <ohl/wallet/config.hpp>
#include <ohl/wallet/wallet_records.hpp>

#define OHL_LIGHT_WALLET_PORT 8899
#define OHL_LIGHT_WALLET_STORAGE_VERSION 1
...
namespace ohl { namespace light_wallet {
   ...
   class light_wallet
   {
      public:
         ...
         ohl::wallet::transaction_ledger_entry summarize(const std::string& account_name, const fc::variant_object& transaction_bundle);
...
FC_REFLECT( ohl::light_wallet::account_data,
            (encrypted_private_key)
            (user_account)
            (transaction_record_cache)
            (transaction_index)
            (scan_cache) );
FC_REFLECT( ohl::light_wallet::light_wallet_data,

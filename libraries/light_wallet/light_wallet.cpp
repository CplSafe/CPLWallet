#include <ohl/light_wallet/light_wallet.hpp>
#include <ohl/blockchain/time.hpp>
#include <ohl/blockchain/balance_operations.hpp>
#include <ohl/blockchain/transaction_creation_state.hpp>
...
namespace ohl { namespace light_wallet {
using std::string;
     ...
     restore(restore_function)
{
   try {
      ...
      if( storage_version != OHL_LIGHT_WALLET_STORAGE_VERSION )
      {
      ...
      persist("storage_version", fc::to_string(OHL_LIGHT_WALLET_STORAGE_VERSION));
...
{
   string last_error;
   auto resolved = fc::resolve( host, port ? port : OHL_LIGHT_WALLET_PORT );
...
{ try {
   ...
   ohl::blockchain::public_key_type recipient_key;
   if( to_account.valid() )
      recipient_key = to_account->active_key();
   else
      recipient_key = ohl::blockchain::public_key_type(to_account_name);

   asset fee = get_fee( symbol );

   fc::time_point_sec expiration = fc::time_point::now() + fc::seconds(OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 30);

   ohl::blockchain::transaction_creation_state creator;
...
oprice light_wallet::get_median_feed_price( const string& symbol )
{
   ...
   if( price_ratio != "" )
   {
      auto base_rec = get_asset_record( OHL_BLOCKCHAIN_SYMBOL );
...
map<string, pair<double,double>> light_wallet::balance(const string& account_name) const
{
   ...
   map<string, pair<double,double>> balances = {{OHL_BLOCKCHAIN_SYMBOL, {0,0}}};
...
vector<ohl::wallet::transaction_ledger_entry> light_wallet::transactions(const string& account_name,
                                                                         const string& symbol)
{
   FC_ASSERT( is_open() );
   ...
   vector<ohl::wallet::transaction_ledger_entry> results;
...
ohl::wallet::transaction_ledger_entry light_wallet::summarize(const string& account_name,
                                                              const fc::variant_object& transaction_bundle)
{ try {
   FC_ASSERT( is_open() );
   FC_ASSERT( is_unlocked() );
   ...
   ohl::wallet::transaction_ledger_entry summary;

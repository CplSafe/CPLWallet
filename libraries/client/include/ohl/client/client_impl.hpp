...
#include <ohl/client/notifier.hpp>
#include <ohl/net/chain_server.hpp>
...
class client_impl : public ohl::net::node_delegate,
      public ohl::api::common_api
{
public:
   ...
   client_impl(ohl::client::client* self, const std::string& user_agent) :
      _self(self),
      ...
      _blockchain_synopsis_size_limit((unsigned)(log2(OHL_BLOCKCHAIN_BLOCKS_PER_YEAR * 20)))
   {
      try
      {
   ...
   // @{
   virtual bool has_item(const ohl::net::item_id& id) override;
   virtual bool handle_message(const ohl::net::message&, bool sync_mode) override;
   virtual std::vector<ohl::net::item_hash_t> get_item_ids(uint32_t item_type,
                                                           const vector<ohl::net::item_hash_t>& blockchain_synopsis,
                                                           uint32_t& remaining_item_count,
                                                           uint32_t limit = 2000) override;
   virtual ohl::net::message get_item(const ohl::net::item_id& id) override;
   virtual fc::sha256 get_chain_id() const override
   {
      FC_ASSERT( _chain_db != nullptr );
      return _chain_db->get_chain_id();
   }
   virtual std::vector<ohl::net::item_hash_t> get_blockchain_synopsis(uint32_t item_type,
   ...
   virtual uint32_t get_block_number(const ohl::net::item_hash_t& block_id) override;
   virtual fc::time_point_sec get_block_time(const ohl::net::item_hash_t& block_id) override;
   virtual fc::time_point_sec get_blockchain_now() override;
   virtual ohl::net::item_hash_t get_head_block_id() const override;
   ...
   ohl::client::client*                                    _self;
   std::string                                             _user_agent;
   ohl::cli::cli*                                          _cli = nullptr;
   ...
   ohl::rpc::rpc_server_ptr                                _rpc_server = nullptr;
   std::unique_ptr<ohl::net::chain_server>                 _chain_server = nullptr;
   std::unique_ptr<ohl::net::upnp_service>                 _upnp_service = nullptr;
   ...
   ohl::net::node_ptr                                      _p2p_node = nullptr;
   bool                                                    _simulated_network = false;

   ohl_gntp_notifier_ptr                                   _notifier;
   ...
   // include all of the method overrides generated by the ohl_api_generator
   // this file just contains a bunch of lines that look like:
   // virtual void some_method(const string& some_arg) override;
#include <ohl/rpc_stubs/common_api_overrides.ipp> //include auto-generated RPC API declarations
};

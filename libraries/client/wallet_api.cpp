...
#include <ohl/utilities/words.hpp>
                                         ...
                                         const string& new_passphrase_verify )
{
    ...
    if( new_passphrase.size() < OHL_WALLET_MIN_PASSWORD_LENGTH )
        FC_CAPTURE_AND_THROW( password_too_short );

    if( brain_key.size() && brain_key.size() < OHL_WALLET_MIN_BRAINKEY_LENGTH )
...
vector<ohl::wallet::account_address_data> detail::client_impl::wallet_list_my_addresses() const
{
...
void read_keys( const fc::variant& vo, vector<private_key_type>& keys, const string& password )
{
    try
    {
      ...
      const auto key = ohl::utilities::wif_to_key( wif_key );
    ...
    catch( const ohl::wallet::invalid_password& e )
    {
        throw;
    }
    catch( ... )
...
{ try {
    ...
    const share_type max_supply = OHL_BLOCKCHAIN_MAX_SHARES;
...
vector<ohl::wallet::escrow_summary> client_impl::wallet_escrow_summary( const string& account_name ) const
{ try {
...
{
  if( fee_symbol.empty() )
     return _wallet->get_transaction_fee( _chain_db->get_asset_id( OHL_BLOCKCHAIN_SYMBOL ) );

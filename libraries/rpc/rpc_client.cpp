...
namespace ohl { namespace rpc { 
  ...
  namespace detail 
  {
    class rpc_client_impl : public ohl::rpc_stubs::common_api_rpc_client
    {
    public:
    ...
    void rpc_client_impl::connect_to(const fc::ip::endpoint& remote_endpoint,
                                     const ohl::blockchain::public_key_type& remote_public_key)
    {
       fc::buffered_istream_ptr buffered_istream;
       fc::buffered_ostream_ptr buffered_ostream;

       if( remote_public_key != ohl::blockchain::public_key_type() )
       {
          net::stcp_socket_ptr socket = std::make_shared<ohl::net::stcp_socket>();

...
namespace ohl { namespace rpc {
  ...
  /**
  *  @class rpc_server
  *  @brief provides a json-rpc interface to the ohl client
  */
  class rpc_server
  {
     public:
       rpc_server(ohl::client::client* client);
       virtual ~rpc_server();

       bool        configure_rpc(const ohl::client::rpc_server_config& cfg);
       bool        configure_http(const ohl::client::rpc_server_config& cfg);
       bool        configure_websockets(const ohl::client::rpc_server_config& cfg);
       bool        configure_encrypted_rpc(const ohl::client::rpc_server_config& cfg);
       ...
       const ohl::api::method_data& get_method_data(const std::string& method_name);
       std::vector<ohl::api::method_data> get_all_method_data() const;
     ...
     protected:
       friend class ohl::rpc::detail::rpc_server_impl;

       void validate_method_data(ohl::api::method_data method);
       void register_method(ohl::api::method_data method);

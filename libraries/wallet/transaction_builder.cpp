...
using namespace ohl::wallet::detail;

void transaction_builder::set_wallet_implementation(std::unique_ptr<ohl::wallet::detail::wallet_impl>& wimpl)
{
    _wimpl = wimpl.get();
}
...
transaction_builder& transaction_builder::deposit_asset_with_escrow(const ohl::wallet::wallet_account_record& payer,
                                                        const ohl::blockchain::account_record& recipient,
                                                        const ohl::blockchain::account_record& escrow_agent,
...
{ try {
   ...
   if( memo.size() > OHL_BLOCKCHAIN_MAX_MEMO_SIZE )
...
{ try {
   ...
   auto age_at_earliest_confirmation = _wimpl->_blockchain->now()
                                       + OHL_BLOCKCHAIN_BLOCK_INTERVAL_SEC
                                       - (*order->expiration - OHL_BLOCKCHAIN_MAX_SHORT_PERIOD_SEC);

...
#include <ohl/blockchain/account_record.hpp>
#include <ohl/blockchain/balance_record.hpp>
#include <ohl/blockchain/extended_address.hpp>
...
namespace ohl { namespace wallet {
   ...
   struct account_data : public ohl::blockchain::account_record
   {
       ...
       // Kept in OHL for upgrading old wallet versions; remove after 1.0
       int8_t           approved = 0;
   };
...
FC_REFLECT_ENUM( ohl::wallet::wallet_record_type_enum,
...
FC_REFLECT_ENUM( ohl::wallet::property_enum,
        (version)
        (next_record_number)
        (next_child_key_index)
        (automatic_backups)
...
FC_REFLECT( ohl::wallet::wallet_property,
        (key)
        (value)
        )

FC_REFLECT_DERIVED( ohl::wallet::account_data, (ohl::blockchain::account_record),
		(is_my_account)
...
FC_REFLECT_DERIVED(ohl::wallet::account_address_data, (ohl::wallet::account_data),
	(owner_address)
	(active_address)
	)
FC_REFLECT( ohl::wallet::master_key,
        (encrypted_key)
        (checksum)
        )

FC_REFLECT( ohl::wallet::key_data,
        (account_address)
        (child_key_index)
        (public_key)
...
FC_REFLECT_ENUM( ohl::wallet::contact_data::contact_type_enum,
        (account_name)
        (public_key)
        (address)
        (btc_address)
...
FC_REFLECT( ohl::wallet::contact_data,
        (contact_type)
        (data)
        (label)
        (custom_data)
...
FC_REFLECT( ohl::wallet::approval_data,
        (name)
        (approval)
        (custom_data)
        )

FC_REFLECT( ohl::wallet::transaction_data,
        (record_id)
        (block_num)
        (is_virtual)
        (is_confirmed)
...
FC_REFLECT( ohl::wallet::ledger_entry,
        (from_account)
        (to_account)
        (amount)
        (memo)
...
FC_REFLECT( ohl::wallet::transaction_ledger_entry,
        (id)
        (block_num)
        (block_id)
        (timestamp)
...
FC_REFLECT_DERIVED( ohl::wallet::pretty_transaction_experimental, (ohl::wallet::transaction_ledger_entry),
        (inputs)
        (outputs)
        (balances)
        (notes)
...
FC_REFLECT( ohl::wallet::setting,
        (name)
        (value)
        )

FC_REFLECT( ohl::wallet::generic_wallet_record,
        (type)
        (data)
        )
...
namespace fc {
   ...
   template<typename T, ohl::wallet::wallet_record_type_enum N>
   struct get_typename< ohl::wallet::wallet_record<T,N> >
   {
   ...
   template<typename Type, ohl::wallet::wallet_record_type_enum N>
   struct reflector<ohl::wallet::wallet_record<Type,N>>
   {
      typedef ohl::wallet::wallet_record<Type,N>  type;
      ...
      {
          {
            typedef decltype(((ohl::wallet::base_record<N>*)nullptr)->wallet_record_index) member_type;
            visitor.TEMPLATE operator()<member_type,ohl::wallet::base_record<N>,&ohl::wallet::base_record<N>::wallet_record_index>( "index" );
          }

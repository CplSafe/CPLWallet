#include <ohl/wallet/config.hpp>
#include <ohl/wallet/exceptions.hpp>
#include <ohl/wallet/wallet.hpp>
#include <ohl/wallet/wallet_impl.hpp>
...
namespace detail {
   ...
   public_key_type wallet_impl::deposit_from_transaction( signed_transaction& transaction, const asset& amount,
                                                          ...
                                                          const string& memo )
   { try {
       if( memo.size() > OHL_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE )
   ...
   void wallet_impl::upgrade_version()
   {
       ...
       if( current_version > OHL_WALLET_VERSION )
       {
           FC_THROW_EXCEPTION( unsupported_version, "Wallet version newer than client supports!",
                               ("wallet_version",current_version)("supported_version",OHL_WALLET_VERSION) );
       }
       else if( current_version == OHL_WALLET_VERSION )
       {
           return;
       }
       ...
       try
       {
           ...
           {
               self->set_transaction_fee( asset( OHL_WALLET_DEFAULT_TRANSACTION_FEE ) );
           }

           if( current_version < 106 )
           {
               self->set_transaction_expiration( OHL_WALLET_DEFAULT_TRANSACTION_EXPIRATION_SEC );
           }
           ...
           {
               self->set_version( OHL_WALLET_VERSION );
                                            ...
                                            )
   {
      ...
      //
      // For example, 1 USD = 10000 USD-satoshi, 1 OHL = 100000 OHL-satoshi
      //
      // Thus, 0.02 USD / OHL expressed as a ratio of satoshis would be
      //       0.02 USD / OHL * (1 OHL / 100000 OHL-satoshi) * (10000 USD-satoshi / 1 USD) = 0.002 USD-satoshi per OHL-satoshi.
      //
   ...
   { try {
      ...
      if( password.size() < OHL_WALLET_MIN_PASSWORD_LENGTH )
   ...
   slate_record wallet_impl::get_delegate_slate( const vote_strategy strategy )const
   {
       ...
       {
           if( record.slate.size() >= OHL_BLOCKCHAIN_MAX_SLATE_SIZE )
               break;
   ...
   { try {
      ...
      try
      {
          ...
          open( wallet_name );
          unlock( password, OHL_WALLET_DEFAULT_UNLOCK_TIME_SEC );
      }
      catch( ... )
      {
   ...
   { try {
      ...
      try
      {
          ...
          close();
          open( wallet_name );
          unlock( passphrase, OHL_WALLET_DEFAULT_UNLOCK_TIME_SEC );
      }
      catch( ... )
      {
   ...
   { try {
      ...
      try
      {
          ...
          const uint32_t max_timeout_seconds = std::numeric_limits<uint32_t>::max() - ohl::blockchain::now().sec_since_epoch();
   ...
   bool	wallet::check_passphrase(const string& passphrase)
   {
	   try {
		   ...
		   if (passphrase.size() < OHL_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
   ...
   { try {
      if( NOT is_open() ) FC_CAPTURE_AND_THROW( wallet_closed );
      if( NOT is_unlocked() ) FC_CAPTURE_AND_THROW( wallet_locked );
      if( new_passphrase.size() < OHL_WALLET_MIN_PASSWORD_LENGTH ) FC_CAPTURE_AND_THROW( password_too_short );
   ...
   { try {
      ...
      auto key = ohl::utilities::wif_to_key( wif_key );
      if( key.valid() )
      {
   ...
   { try {
      ...
      const auto version = ohl::client::version_info()["client_version"].as_string();
   ...
   { try {
       ...
       for( const auto& item : items )
       {
           ...
           if( name.find( OHL_ADDRESS_PREFIX ) != 0 )
               continue;
   ...
   { try {
      ...
      if( amount.asset_id != asset_id_type( 0 ) )
      {
          ...
          catch( const ohl::wallet::insufficient_funds& )
          {
...
//      required_signatures.insert( issuer_account->active_key() );
  ...
  //    if( sign )
   ...
   { try {
      ...
      asset fee( OHL_WALLET_DEFAULT_TRANSACTION_FEE, 0 );
      try
      {
   ...
   { try {
       ...
       if( secs > OHL_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC )
